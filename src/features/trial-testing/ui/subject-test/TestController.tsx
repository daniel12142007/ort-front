...
const TestController = () => {
	...
	const [myChose, setChose] = React.useState<AnswerState>()
	const { fetchQuestions, questions, count, setMyAnswer, loading } = useTrialTestStore()

	React.useEffect(() => {
		const id = location.state?.id
		console.log(id)
		if (!id) navigate("/main/trial-testing")
		fetchQuestions(id, 10)
	}, [])
	...
	const testFinish = async () => {
		if (myChose) {
			const res = await setMyAnswer(myChose)
			if (res === "success") {
				navigate("/main/trial-testing/finish", {
					state: { resultId: myChose?.testId },
				})
			}
		} else {
			setModal(true)
		...
		<div className="relative">
			{loading ? (
				...
				<div className="flex flex-col w-4/5 m-auto">
					...
					<div className="w-full bg-white rounded-lg p-5">
								...
								{questions.map((question, i) => (
									<div className={`${i === current ? "" : "hidden"}`} key={i}>
										<div className="flex flex-col gap-2">
											...
											<div
												className="border border-[#eaeaea] rounded-lg p-2 bg-[#f9f9f9] flex gap-5">
												...
												>
													<div className="flex items-center gap-3">
														<input
															type="radio"
															name="option"
															checked={myChose?.optionId === option.id}
														/>
						...
						<div className="flex justify-evenly items-center">
							<button
								className="py-2 px-5 bg-[#ff9c40] rounded-lg text-white font-[500]
                text-lg cursor-pointer shadow-lg transition-all duration-200 active:scale-95
                hover:opacity-85 disabled:opacity-50 disabled:cursor-not-allowed"
							...
							{current !== count - 1 && (
								<button
									className="py-2 px-5 bg-[#407bff] rounded-lg text-white font-[500]
                  text-lg cursor-pointer shadow-lg transition-all duration-200 active:scale-95
                  hover:opacity-85 disabled:opacity-50 disabled:cursor-not-allowed"
									onClick={() => changePage()}
									disabled={!myChose}
								>
									{!!myChose && current === count - 1 ? "Завершить" : "Далее"}
								</button>
							)}
						</div>
					</div>
